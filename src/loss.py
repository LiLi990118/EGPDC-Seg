...
class MultiClassDiceLoss(nn.Module):
    ...
    def forward(self, input, target, idx,weights):
        ...
        print(allLoss)
        count_of_0 = weights.count(0)
        # if idx==0:
        #     return totalLoss / (C-3), totalDice / (C-3)
        # else:
        #     return totalLoss / C, totalDice / C
        return totalLoss / (C-count_of_0), totalDice / (C-count_of_0)
...
class FocalLoss_12(nn.Module):
    def __init__(self, num_class=5, alpha=None, gamma=2, ignore_index=None, reduction='mean'):
        super(FocalLoss_12, self).__init__()
        self.num_class = num_class
        self.gamma = gamma
        self.reduction = reduction
        self.smooth = 1e-4
    ...
    def forward(self, logit, target):
        ...
        # 创建掩码，只考虑标签为1和2的情况
        # 确保只有当目标标签是 1 或 2 时，损失才会被计算
        class_mask = (target == 1) | (target == 2)
        ...
        if self.ignore_index is not None:
            valid_mask = target != self.ignore_index
            target = target * valid_mask
        target = target.long() * class_mask.long()  # 只保留标签为1和2的
        ...
        # 只计算标签为1和2的损失
        loss = loss * class_mask.squeeze()

        if self.reduction == 'mean':
            loss = loss.mean()
            if valid_mask is not None:
                loss = loss.sum() / class_mask.sum()
        elif self.reduction == 'none':
            loss = loss.view(target.size(0))

        print('focal loss12:', loss.item())
        return loss
