...
from .LoadData_one import LoadDatasets
from .models import *
from .utils import *
from .client_ParamDis2_Pancreas_alig import Client
...
class Server(object):
    ...
    def create_clients(self, A_DATA, B_DATA, C_DATA, val_A, val_B, val_C):
        ...
        client0 = Client(client_id=0, local_data=A_DATA, local_valdata=val_A, device=self.device,
                         save_img_path=self.save_img_path)
        clients.append(client0)
        client1 = Client(client_id=1, local_data=B_DATA, local_valdata=val_B, device=self.device,
                         save_img_path=self.save_img_path)
        clients.append(client1)
        client2 = Client(client_id=2, local_data=C_DATA, local_valdata=val_C, device=self.device,
                         save_img_path=self.save_img_path)
    ...
    def mp_evaluate_selected_models(self, selected_index):
        ...
        if self.val_dice[selected_index] > self.max_dice[selected_index]:
            print('333')
            logger.info(
                '\t Saving best model, mean dice increased from: {:.4f} to {:.4f}'.format(self.max_dice[selected_index],
                                                                                          self.val_dice[
                                                                                              selected_index]))
        ...
        with torch.no_grad():
            for dataset, labelName in self.dataloader:
                ...
                for i in range(mask.shape[0]):
                    ...
                    global_dir_path = img_path + labelName[i][:-4]
                    if not os.path.exists(global_dir_path):
                        os.makedirs(global_dir_path)
                    img.save(global_dir_path + ".png")
    ...
    def fit(self):
        ...
        model_path = './global_model_FedParamDis2_Pancreas_alig/FLARETrain_1234_222'
        max_dice = 0
        min_loss = 999
